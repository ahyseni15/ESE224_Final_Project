...
BookBST* BookBST::deleteNode(BookBST* root, Book b)
{
    if (root == NULL)
    {
        return root;
    ...
    else
    {
        ...
        {
            BookBST* temp = root->right;
            free(root);
            return temp;
        }
        else if (root->right == NULL)
        {
            BookBST* temp = root->left;
            free(root);
            return temp;
        }
        BookBST* temp = minValueNode(root->right);
        root->b = temp->b;
        root->right = deleteNode(root->right, temp->b);
    }
    return root;
}

BookBST* BookBST::minValueNode(BookBST* node)
{
    BookBST* current = node;
